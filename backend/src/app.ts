...
// 🔍 Validación de env
// -------------------------------
const requiredEnv = ["MONGO_URI", "SESSION_SECRET", "FRONTEND_URL"];
const missing = requiredEnv.filter((v) => !process.env[v]);
if (missing.length > 0) {
  console.error("❌ ENV faltantes:", missing);
  process.exit(1);
}

// Render usa proxy → necesario para cookies secure
...
// -------------------------------
// ❗ ERROR 404 (solo fuera de producción)
// -------------------------------
if (process.env.NODE_ENV !== "production") {
  app.use((req, res) => {
    res.status(404).json({
